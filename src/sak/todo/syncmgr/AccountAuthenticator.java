...
public class AccountAuthenticator extends AbstractAccountAuthenticator {
	...
	public static String UserName = "dummyUserNameForTest";
	
	/** this is a key for email*/
    ...
    public Bundle getAuthToken(AccountAuthenticatorResponse response, Account account,
        ...
        final String email = am.getUserData(account, EMAIL_KEY);
        if (password != null) {
            final String authToken = NetworkUtilities.authenticate(account.name, password, email);
            if (!TextUtils.isEmpty(authToken)) {
                final Bundle result = new Bundle();
                result.putString(AccountManager.KEY_ACCOUNT_NAME, account.name);
                result.putString(AccountManager.KEY_ACCOUNT_TYPE, Constants.ACCOUNT_TYPE);
                result.putString(AccountManager.KEY_AUTHTOKEN, authToken);
                result.putString(AccountAuthenticator.EMAIL_KEY, email);
                return result;
            }
        }
        ...
        final Intent intent = new Intent(mContext, AuthenticatorActivity.class);
        // Putting account details to pass it to the authentication activity
        intent.putExtra(AuthenticatorActivity.PARAM_USERNAME, account.name);
        intent.putExtra(AuthenticatorActivity.PARAM_AUTHTOKEN_TYPE, authTokenType);
        intent.putExtra(AuthenticatorActivity.PARAM_EMAIl, email);
